...
export class HealthMonitor extends EventEmitter {
  ...
  private setupEventListeners(): void {
    this.serviceRegistry.on("serviceHealthFailed", (instance, error) => {
      ...
      this.serviceUpGauge.set(
        { service_name: instance.name, instance_id: instance.id },
        0,
      );
    });
  ...
  addHealthCheck(healthCheck: HealthCheck): void {
    ...
    logger.info(
      `Health check added for ${healthCheck.serviceName} at ${healthCheck.endpoint}`,
    );
  }
  ...
  private async performHealthCheck(healthCheck: HealthCheck): Promise<void> {
    try {
      const instances = await this.serviceRegistry.getAllServices(
  ...
  private updateServiceMetrics(
    ...
    this.serviceErrorRateGauge.set(
      ...
      metrics.totalInstances > 0
        ? metrics.unhealthyInstances / metrics.totalInstances
        : 0,
    );
  }
  ...
  private createAlert(alert: Alert): void {
    ...
    logger.warn(
      `Alert created: ${alert.type} for ${alert.serviceName} - ${alert.message}`,
    );
  }
  ...
  private getServiceAlerts(serviceName: string): Alert[] {
    return Array.from(this.alerts.values()).filter(
      (alert) => alert.serviceName === serviceName && !alert.resolved,
    );
  }
  ...
  async getOverallHealthStatus(): Promise<Record<string, any>> {
    ...
    for (const serviceName of serviceNames) {
      ...
      const serviceMetrics: ServiceHealthMetrics = {
        ...
        unhealthyInstances: instances.filter((i) => i.health === "unhealthy")
          .length,
  ...
  getActiveAlerts(severity?: string): Alert[] {
    return Array.from(this.alerts.values()).filter(
      (alert) => !alert.resolved && (!severity || alert.severity === severity),
    );
  }

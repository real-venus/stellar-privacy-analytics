...
class ValidationService {
  async validateCertification(data: ValidationData): Promise<ValidationRecord> {
    try {
      ...
      logger.info(
        `Validated certification ${data.certificationId} by ${data.validator}`,
      );
  ...
  async submitThirdPartyValidation(
    ...
    try {
      ...
      const validationResult = await this.callThirdPartyValidator(
        certificationId,
        validatorName,
        evidence,
      );
      ...
      const newStatus =
        validationResult.status === "approved" ? "validated" : "pending";
      await certificationService.updateCertificationStatus(
        certificationId,
        newStatus,
        validatorName,
      ...
      logger.info(
        `Third-party validation completed for ${certificationId} by ${validatorName}`,
      );
      return validationRecord;
    } catch (error) {

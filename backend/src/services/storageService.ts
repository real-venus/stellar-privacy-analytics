...
export class StorageService {
  ...
  async store(
    ...
    if (defaultBackend) {
      const result = await defaultBackend.upload(
        processedData,
        fileName,
        options,
      );
    ...
    for (
      let i = 0;
      i < Math.min(replicaCount - 1, availableBackends.length - 1);
      i++
    ) {
      const backend = availableBackends.find(
        (b) => !results.some((r) => r.backend === b.name),
      );
      if (backend) {
        try {
          ...
          logger.error(`Replication failed for backend: ${backend.name}`, {
            error: error.message,
          });
        }
      }
    ...
    for (const result of results) {
      try {
        ...
        if (currentHash !== result.hash) {
          logger.error(
            `Integrity verification failed for ${result.uri} on ${result.backend}`,
          );
          continue;
        }
        ...
        logger.error(`Retrieval failed from ${result.backend}`, {
          error: error.message,
        });
      }
    }

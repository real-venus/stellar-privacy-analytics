...
export class PrivacyRiskAssessmentService {
  ...
  async assessWorkflowRisk(
    ...
    try {
      ...
      logger.info(
        `Risk assessment completed for workflow ${workflow.id}: ${category} risk (${overallScore})`,
      );
      return assessment;
    } catch (error) {
  ...
  private async evaluateRiskFactors(
    ...
    const processingRisk = this.evaluateProcessingScope(
    ...
    const thirdPartyRisk = this.evaluateThirdPartyRisk(
      workflow.processingSteps,
    );
    ...
    for (const customFactor of this.criteria.customFactors) {
      const customRisk = await this.evaluateCustomFactor(
        workflow,
        customFactor,
      );
      if (customRisk) {
  ...
  private evaluateProcessingScope(
    processingSteps: ProcessingStep[],
  ): RiskFactor {
    let totalRisk = 0;
    ...
    for (const step of processingSteps) {
      ...
      if (
        step.type === "sharing" &&
        step.thirdParties.some((p) => p.includes("international"))
      ) {
  ...
  private evaluateSecurityMeasures(workflow: DataWorkflow): RiskFactor {
    ...
    if (workflow.anonymizationTechniques.length > 0) {
      securityScore -=
        0.1 * Math.min(workflow.anonymizationTechniques.length, 3);
    }
  ...
  private evaluateComplianceRisk(workflow: DataWorkflow): RiskFactor {
    ...
    if (
      workflow.crossBorderTransfer &&
      !workflow.dataSubjects.includes("eu_data_subjects")
    ) {
      complianceScore += 0.2;
  ...
  private evaluateThirdPartyRisk(
    processingSteps: ProcessingStep[],
  ): RiskFactor {
  ...
  private async evaluateCustomFactor(
    workflow: DataWorkflow,
    customFactor: CustomRiskFactor,
  ...
  private estimateEffort(mitigation: string): "low" | "medium" | "high" {
    ...
    if (
      highEffortKeywords.some((keyword) =>
        mitigation.toLowerCase().includes(keyword),
      )
    ) {
      return "high";
    }
    if (
      lowEffortKeywords.some((keyword) =>
        mitigation.toLowerCase().includes(keyword),
      )
    ) {
      return "low";
  ...
  private async evaluateGDPRCompliance(
    workflow: DataWorkflow,
  ): Promise<ComplianceFramework> {
  ...
  private async evaluateCCPACompliance(
    workflow: DataWorkflow,
  ): Promise<ComplianceFramework> {
    const requirements: ComplianceRequirement[] = [
      {
  ...
  private generateRecommendations(
    ...
    if (highRiskFactors.length > 0) {
      recommendations.push(
        "Immediate attention required for high-risk factors",
      );
    }

    // Prioritized mitigation strategies
    const urgentStrategies = mitigationStrategies.filter(
      (s) => s.priority === "urgent",
    );
    if (urgentStrategies.length > 0) {
      recommendations.push(
        `Implement ${urgentStrategies.length} urgent mitigation strategies immediately`,
      );
    }

    // Compliance recommendations
    const complianceRisks = riskFactors.filter(
      (f) => f.category === "compliance" && f.score > 0.5,
    );
    if (complianceRisks.length > 0) {
      recommendations.push(
        "Address compliance gaps to avoid regulatory penalties",
      );
    }

    // Security recommendations
    const securityRisks = riskFactors.filter(
      (f) => f.category === "security_measures" && f.score > 0.6,
    );
  ...
  private async getHistoricalTrend(workflowId: string): Promise<RiskTrend[]> {
    try {
      ...
      logger.error(
        `Error fetching historical trend for workflow ${workflowId}:`,
        error,
      );
      return [];
    }
  ...
  async getAssessmentHistory(
    ...
    try {
      ...
      logger.error(
        `Error fetching assessment history for workflow ${workflowId}:`,
        error,
      );
      return [];
    }
  ...
  async updateAssessmentCriteria(
    criteria: Partial<RiskAssessmentCriteria>,
  ): Promise<void> {
    try {

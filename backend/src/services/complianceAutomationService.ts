...
export class ComplianceAutomationService {
    ...
    const gdpr: ComplianceRegulation = {
      ...
      rules: [
        ...
        },
        {
          ...
          remediation:
            "Set up automated breach detection and notification system",
        },
        {
          id: "gdpr-005",
          regulationId: "gdpr",
      ...
      rules: [
        {
          ...
          remediation:
            "Implement data disclosure mechanism for consumer requests",
        },
        {
          id: "ccpa-002",
          regulationId: "ccpa",
        ...
        },
        {
          ...
          remediation:
            'Add "Do Not Sell My Personal Information" link and mechanism',
        },
        {
          id: "ccpa-004",
          regulationId: "ccpa",
          ...
          parameters: {
            prohibitPriceDifference: true,
            prohibitServiceDenial: true,
          },
      ...
      rules: [
        {
          ...
          remediation:
            "Implement comprehensive administrative safeguards program",
        },
        {
          id: "hipaa-002",
          regulationId: "hipaa",
  ...
  private async generateAlert(
    ...
    const alert: ComplianceAlert = {
      id: `alert_${Date.now()}`,
      severity: criticalViolations.some((v) => v.severity === "critical")
        ? "critical"
        : "high",
  ...
  async getComplianceDashboard(): Promise<{
    ...
    const overallScore =
      regulationStats.length > 0
        ? Math.round(
            regulationStats.reduce((sum, r) => sum + r.score, 0) /
              regulationStats.length,
          )
        : 0;
  ...
  async generateComplianceReport(regulationId: string): Promise<{
    ...
    const scans = this.scanHistory.filter(
      (s) => s.regulation === regulation.name,
    );
    const now = new Date();
    ...
    const totalViolations = recentScans.reduce(
      (sum, s) => sum + s.violations.length,
      0,
    );
    const criticalViolations = recentScans.reduce(
      (sum, s) =>
        sum + s.violations.filter((v) => v.severity === "critical").length,
      0,
    );
    ...
    const averageScore =
      recentScans.length > 0
        ? Math.round(
            recentScans.reduce((sum, s) => sum + s.score, 0) /
              recentScans.length,
          )
        : 0;
    ...
    recentScans.forEach((scan) => {
      scan.violations.forEach((v) => {
        violationCounts.set(
          v.ruleName,
          (violationCounts.get(v.ruleName) || 0) + 1,
        );
      });
    });

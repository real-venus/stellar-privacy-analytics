...
runner.test("ThresholdCryptography: Create shares successfully", async () => {
  ...
  assertEqual(
    shares[0].holder,
    "holder1",
    "First share should belong to holder1",
  );
});
...
runner.test(
  ...
  async () => {
    ...
    const combo1 = await crypto.reconstructSecret(
      [shares[0], shares[2], shares[4]],
      threshold,
    );
    assertBufferEqual(combo1, secret, "Combo 1 should reconstruct secret");

    const combo2 = await crypto.reconstructSecret(
...
runner.test("ThresholdCryptography: Verify valid share", async () => {
  const crypto = new ThresholdCryptography();
  const secret = randomBytes(32);
  const shares = await crypto.createShares(secret, 3, 5, [
    "h1",
    "h2",
    "h3",
    "h4",
...
runner.test(
  ...
  async () => {
    ...
    const originalShares = await crypto.createShares(
      secret,
      threshold,
      totalShares,
      shareHolders,
    );
    const refreshedShares = await crypto.refreshShares(
      originalShares,
      threshold,
    );
    ...
    assert(
      refreshedShares[0].share !== originalShares[0].share,

...
runner.test("Service Initialization", async () => {
  ...
  assert(
    services.keyManagementService !== null,
    "KeyManagementService should initialize",
  );
  assert(
    services.thresholdCrypto !== null,
    "ThresholdCryptography should initialize",
  );
...
runner.test("Threshold Cryptography: Full Workflow", async () => {
  ...
  const reconstructedRefreshed = await crypto.reconstructSecret(
    refreshed.slice(0, 3),
    threshold,
  );
  assert(
    reconstructedRefreshed.equals(secret),
    "Refreshed shares should reconstruct to same secret",
  );
...
runner.test("Key Sharing: Access Request Workflow", async () => {
  ...
  // Share the key
  const shares = await sharingService.shareKey(
    keyId,
    keyMaterial,
    threshold,
    holders,
...
runner.test("Backup Service: Statistics", async () => {
  ...
  assert(
    typeof stats.completedBackups === "number",
    "Should have completedBackups",
  );
...
runner.test("HSM Service: System Status", async () => {
  ...
  assert(
    typeof status.killSwitchActive === "boolean",
    "Should have killSwitchActive",
  );
  assert(
    typeof status.activeKeysCount === "number",
    "Should have activeKeysCount",
  );
...
runner
  ...
  .then((success) => {
    if (success) {
      ...
      console.log(
        "📝 Note: Some tests use mock data since HSM is not configured.",
      );

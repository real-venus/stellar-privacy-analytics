...
export class ServiceMesh extends EventEmitter {
  ...
  async request<T = any>(
    ...
    try {
      const serviceInstance = await this.serviceRegistry.getService(
        serviceName,
        true,
      );
      ...
      const circuitBreaker =
        this.circuitBreakerRegistry.getCircuitBreaker(serviceName);
    ...
    for (let attempt = 1; attempt <= this.config.retryAttempts; attempt++) {
      try {
        ...
        if (attempt < this.config.retryAttempts && this.shouldRetry(error)) {
          ...
          logger.warn(
            `Request to ${serviceInstance.name} failed, retrying in ${delay}ms (attempt ${attempt}/${this.config.retryAttempts})`,
            error,
          );
  ...
  getServiceMetrics(serviceName?: string): Record<string, any> {
    ...
    const servicesToCheck = serviceName
      ? [serviceName]
      : Array.from(this.requestMetrics.keys());
    ...
    for (const service of servicesToCheck) {
      ...
      if (recentMetrics.length > 0) {
        const successMetrics = recentMetrics.filter(
          (m) => m.status === "success",
        );
  ...
  async getServiceHealth(): Promise<Record<string, any>> {
    ...
    for (const service of services) {
      if (!health[service.name]) {
        health[service.name] = {
          instances: [],
          circuitBreakerState: this.circuitBreakerRegistry
            .getCircuitBreaker(service.name)
            .getState(),
        };
      }
  ...
  async get<T = any>(
    serviceName: string,
    path: string,
    options: Partial<ServiceRequest> = {},
  ): Promise<ServiceResponse<T>> {

...
export class MockPaymentService {
  ...
  async createMockPayment(data: {
    ...
    const failureType = shouldFail
      ? data.failureType || this.getRandomFailureType()
      : undefined;

    const payment: MockPayment = {
      paymentId,
      subscriptionId: data.subscriptionId,
      amount: sandboxConfig.isFeatureEnabled("zeroValueTokens")
        ? 0
        : data.amount,
      ...
      failureType,
      transactionHash: shouldFail
        ? undefined
        : this.generateMockTransactionHash(),
  ...
  async getMockPaymentHistory(params: {
    ...
    if (subscriptionId) {
      ...
      const paymentKeys = allKeys.filter(
        (key) => key.includes(":payment:") && !key.includes(":subscription:"),
      );
  ...
  async getPaymentById(paymentId: string): Promise<MockPayment | null> {
    const paymentData = await this.redisClient.get(
      `sandbox:payment:${paymentId}`,
    );
  ...
  private getRandomFailureType():
    | "insufficient_funds"
    | "network_error"
    | "timeout"
    | "invalid_signature" {
    const failureTypes: Array<
      "insufficient_funds" | "network_error" | "timeout" | "invalid_signature"
    > = ["insufficient_funds", "network_error", "timeout", "invalid_signature"];
  ...
  private async emitSubscriptionBilledEvent(
    payment: MockPayment,
  ): Promise<void> {
    try {
      const billedEvent = {
      ...
      logger.error("Failed to emit subscription billed event", {
        paymentId: payment.paymentId,
        error,
      });
    }
  ...
  async getPaymentStatistics(subscriptionId?: string): Promise<{
    ...
    const stats = {
      ...
      successfulPayments: history.payments.filter((p) => p.status === "success")
        .length,
      failedPayments: history.payments.filter((p) => p.status === "failed")
        .length,
      pendingPayments: history.payments.filter((p) => p.status === "pending")
        .length,
      totalAmount: history.payments
    ...
    stats.failureRate =
      stats.totalPayments > 0
        ? (stats.failedPayments / stats.totalPayments) * 100
        : 0;
  ...
  async clearMockPaymentData(subscriptionId?: string): Promise<void> {
    if (subscriptionId) {
      ...
      logger.info("Mock payment data cleared for subscription", {
        subscriptionId,
      });
    } else {
      // Clear all mock payment data
      const paymentKeys = await this.redisClient.keys("sandbox:payment:*");
      const subscriptionKeys = await this.redisClient.keys(
        "sandbox:subscription:*:payments",
      );

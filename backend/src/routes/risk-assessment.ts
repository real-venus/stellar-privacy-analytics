...
router.post(
  ...
  [
    ...
    body("workflow.dataTypes")
      .isArray()
      .withMessage("Data types must be an array"),
    body("workflow.processingSteps")
      .isArray()
      .withMessage("Processing steps must be an array"),
    body("workflow.retentionPeriod")
      .isInt({ min: 0 })
      .withMessage("Retention period must be a positive integer"),
    body("workflow.encryptionLevel")
      .isIn(["none", "basic", "standard", "advanced"])
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const assessment = await riskAssessmentService.assessWorkflowRisk(
        workflow,
        assessorId,
      );
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.error(
        `Error fetching assessment history for workflow ${req.params.workflowId}:`,
        error,
      );
      res.status(500).json({
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const heatMapData = await riskAssessmentService.generateRiskHeatMap(
...
router.put(
  "/criteria",
  [
    body("dataSensitivityWeights")
      .optional()
      .isObject()
      .withMessage("Data sensitivity weights must be an object"),
    body("processingWeights")
      .optional()
      .isObject()
      .withMessage("Processing weights must be an object"),
    body("securityWeights")
      .optional()
      .isObject()
      .withMessage("Security weights must be an object"),
    body("complianceWeights")
      .optional()
      .isObject()
      .withMessage("Compliance weights must be an object"),
    body("customFactors")
      .optional()
      .isArray()
      .withMessage("Custom factors must be an array"),
  ],
  handleValidationErrors,
  async (req: Request, res: Response) => {
    try {
...
router.post(
  ...
  [
    body("workflows")
      ...
      .withMessage("Workflows must be an array with 1-50 items"),
    body("assessorId").notEmpty().withMessage("Assessor ID is required"),
  ],
  handleValidationErrors,
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info(
        `Starting batch risk assessment for ${workflows.length} workflows`,
      );

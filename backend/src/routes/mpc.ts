...
import {
  MPCOperation,
  MPCSessionStatus,
} from "../services/mpc/shamir-secret-sharing";
...
router.post(
  ...
  [
    ...
    body("threshold")
      .optional()
      .isInt({ min: 2 })
      .withMessage("Threshold must be at least 2"),
  ],
  async (req: Request, res: Response) => {
    try {
      ...
      const session = await mpcNode!.initializeSession(
        sessionId,
        participants,
        operation as MPCOperation,
      );

      // Log to Stellar
      let stellarTxId = "";
      try {
        stellarTxId = await stellarLogger!.logSessionStart(
          sessionId,
          participants,
          operation,
        );
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      try {
        await stellarLogger!.logParticipantEvent(
          sessionId,
          mpcNode!["nodeId"],
          "join",
        );
...
router.delete("/session/:sessionId", async (req: Request, res: Response) => {
  try {
    ...
    try {
      await stellarLogger!.logSessionFailure(
        sessionId,
  ...
  if (mpcNode) {
    ...
    mpcNode.on("computationProgress", (sessionId, progress) => {
      mpcNamespace
        ...
        .emit("computation-progress", progress);
    });

    mpcNode.on("computationComplete", (sessionId, result) => {
      mpcNamespace
        .to(`session-${sessionId}`)
        .emit("computation-complete", result);
    });

...
router.put(
  ...
  [
    body("dataRetentionDays")
      ...
      .withMessage("Data retention days must be between 1 and 2555"),
    body("autoDeleteEnabled").optional().isBoolean(),
    body("gdprComplianceEnabled").optional().isBoolean(),
    validateRequest,
  ],
  asyncHandler(async (req, res) => {
    const { dataRetentionDays, autoDeleteEnabled, gdprComplianceEnabled } =
      req.body;
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const retainForLegal = !(
      req.query.retainForLegal === "false" || req.query.retainForLegal === "0"
    );
    ...
    try {
      ...
      const userDatasets = userData.metadata.filter(
        (m) =>
          m.sanitizedMetadata?.userId === userId ||
          m.originalMetadata?.userId === userId,
      );
    ...
    try {
      ...
      res.json({
        ...
        recommendations: {
          ...
          suggestedAction:
            dataApproachingExpiry.total > 100
              ? "immediate_cleanup"
              : "schedule_cleanup",

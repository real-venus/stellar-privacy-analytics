...
router.get("/regulations", async (req: Request, res: Response) => {
  try {
    ...
    router.get(
      "/regulations/:id",
      [param("id").trim().isLength({ min: 1, max: 128 }), validateRequest],
    ...
    router.post(
      ...
      [
        body("regulationId")
          ...
          .notEmpty()
          .isLength({ max: 128 })
          .withMessage("regulationId is required"),
        validateRequest,
      ],
      async (req: Request, res: Response) => {
        try {
          ...
          const result =
            await complianceAutomationService.runComplianceScan(regulationId);
    ...
    router.post("/scan/all", async (req: Request, res: Response) => {
      try {
        ...
        for (const regulation of regulations) {
          try {
            const result = await complianceAutomationService.runComplianceScan(
              regulation.id,
            );
            results.push(result);
          } catch (error) {
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          const auditTrail =
            await complianceAutomationService.getAuditTrail(filters);
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          const report =
            await complianceAutomationService.generateComplianceReport(
              regulationId,
            );
...
router.get("/report/:regulationId", async (req: Request, res: Response) => {
  try {
    const { regulationId } = req.params;
    const report =
      await complianceAutomationService.generateComplianceReport(regulationId);

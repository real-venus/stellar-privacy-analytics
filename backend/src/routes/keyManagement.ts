...
router.post(
  "/keys/:keyId/rotate",
  [param("keyId").isString(), body("reason").optional().isString()],
  handleValidationErrors,
  async (req: Request, res: Response) => {
    try {
...
router.get(
  ...
  [
    query("keyType")
      .optional()
      .isIn(["master", "data", "session", "smpc", "zkp"]),
    query("status")
      .optional()
      .isIn(["active", "rotating", "deprecated", "revoked", "compromised"]),
 ...
 */
router.post(
  "/keys/:keyId/reconstruct",
  [param("keyId").isString(), body("shares").isArray({ min: 2 })],
  handleValidationErrors,
  async (req: Request, res: Response) => {
    try {
      const { keyId } = req.params;
      ...
      const keyMaterial = await keyManagementService!.reconstructKey(
        keyId,
        shares,
      );

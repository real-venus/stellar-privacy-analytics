 ...
 */
router.post(
  "/warming/strategies/:name/execute",
  ...
  try {
    ...
    res.status(500).json({
      ...
      error:
        error instanceof Error
          ? error.message
          : "Failed to update warming strategy",
    });
  }
});
...
router.post("/test/scenario/:name", async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error:
        error instanceof Error ? error.message : "Failed to run test scenario",
    });
  }
});

...
export function createQueueMonitoringRoutes(
  ...
  router.get("/health", async (req: Request, res: Response) => {
    try {
      ...
      const healthPercentage =
        totalWorkers > 0 ? (healthyWorkers / totalWorkers) * 100 : 0;
  ...
  router.get("/metrics/prometheus", async (req: Request, res: Response) => {
    try {
      ...
      metrics.push(
        `# HELP queue_workers_by_status Number of workers by status`,
      );
      ...
      if (queueStats) {
        ...
        if (queueStats.byPriority) {
          metrics.push(
            `# HELP queue_jobs_by_priority Number of jobs by priority`,
          );
          metrics.push(`# TYPE queue_jobs_by_priority gauge`);
          Object.entries(queueStats.byPriority).forEach(
            ([priority, stats]: [string, any]) => {
              metrics.push(
                `queue_jobs_by_priority{priority="${priority}",status="waiting"} ${stats.waiting}`,
              );
              metrics.push(
                `queue_jobs_by_priority{priority="${priority}",status="active"} ${stats.active}`,
              );
            },
          );
        }
      ...
      metrics.push(
        `system_cpu_usage ${stats.systemResources.cpuUsage.toFixed(2)}`,
      );

      metrics.push(`# HELP system_memory_usage Memory usage percentage`);
      metrics.push(`# TYPE system_memory_usage gauge`);
      metrics.push(
        `system_memory_usage ${stats.systemResources.memoryUsage.toFixed(2)}`,
      );

...
router.post(
  ...
  [
    body("environment")
      .isIn(["mainnet", "testnet", "sandbox"])
      .withMessage("environment must be mainnet, testnet, or sandbox"),
  ],
  (req, res) => {
    try {
      const errors = validationResult(req);
      ...
      logger.info("Stellar environment switched", {
        environment,
        requestedBy: req.user?.id,
      });
...
router.post(
  ...
  [
    ...
    body("shouldFail")
      .optional()
      .isBoolean()
      .withMessage("shouldFail must be a boolean"),
    body("failureType")
      .optional()
      .isIn([
        "insufficient_funds",
  ...
  async (req, res) => {
    try {
      ...
      const result =
        await mockPaymentService.createMockPayment(mockPaymentData);
...
router.post(
  ...
  [
    ...
    body("billingPeriodStart")
      .isISO8601()
      .withMessage("billingPeriodStart must be a valid ISO date"),
    body("billingPeriodEnd")
      .isISO8601()
      .withMessage("billingPeriodEnd must be a valid ISO date"),
    body("paymentStatus")
      .optional()
  ...
  async (req, res) => {
    try {
      ...
      const result =
        await sandboxService.simulateSubscriptionBilledEvent(billingEventData);
...
router.post(
  ...
  [
    ...
    body("gracePeriodDays")
      .isInt({ min: 1, max: 30 })
      .withMessage("gracePeriodDays must be between 1 and 30"),
...
router.post(
  ...
  [
    ...
    body("dunningLevel")
      .isInt({ min: 1, max: 5 })
      .withMessage("dunningLevel must be between 1 and 5"),
    body("contactMethod")

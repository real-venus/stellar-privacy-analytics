...
router.post(
  ...
  [
    ...
    body("certificationType")
      .isIn(["GDPR", "CCPA", "HIPAA", "ISO27001", "SOC2", "CUSTOM"])
      .withMessage("Valid certification type is required"),
    ...
    body("privacyLevel")
      ...
      .withMessage("Valid privacy level is required"),
    body("complianceChecks")
      .isArray()
      .withMessage("Compliance checks must be an array"),
  ],
  ...
  async (req, res) => {
    try {
      ...
      const certification =
        await certificationService.generateCertification(certificationData);

      logger.info(
        `Certification generated: ${certification.id} for analysis ${req.body.analysisId}`,
      );
...
router.get(
  ...
  [
    ...
    query("status")
      .optional()
      .isIn(["pending", "validated", "expired", "revoked"]),
    ...
    try {
      const filters = {
        ...
        status: req.query.status as
          ...
          | "revoked"
          | undefined,
        certificationType: req.query.certificationType as
          | "GDPR"
          | "CCPA"
          | "HIPAA"
          | "ISO27001"
      ...
      const certifications =
        await certificationService.getOrganizationCertifications(filters);
...
router.post(
  ...
  [
    param("id").isUUID().withMessage("Valid certification ID is required"),
    body("thirdPartyValidator")
      .isString()
      .trim()
      .isLength({ min: 1, max: 255 })
      .withMessage("Third party validator is required"),
    body("validationEvidence")
      .isArray()
      .withMessage("Validation evidence must be an array"),
  ],
  handleValidationErrors,
  async (req, res) => {
    try {
      ...
      const validation =
        await validationService.validateCertification(validationData);

      logger.info(
        `Certification validated: ${req.params.id} by ${req.body.thirdPartyValidator}`,
      );
...
router.post(
  ...
  [
    param("id").isUUID().withMessage("Valid certification ID is required"),
    body("checkType")
      .isIn(["automated", "manual", "third-party"])
      .withMessage("Valid check type is required"),
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const badge = await certificationService.generateBadge(
        req.params.id,
        badgeOptions,
      );
      ...
      if (badgeOptions.format === "json") {
        ...
        res.setHeader(
          "Content-Type",
          badgeOptions.format === "svg" ? "image/svg+xml" : "image/png",
        );
        res.send(badge);
      }
    } catch (error) {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result =
        await certificationService.revokeCertification(revocationData);

      logger.info(
        `Certification revoked: ${req.params.id} by ${req.body.revokedBy}`,
      );
...
router.get(
  "/public/verify/:verificationCode",
  [
  ...
  async (req, res) => {
    try {
      const verification = await certificationService.verifyPublicCertification(

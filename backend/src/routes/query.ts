...
const queryPayloadRules = [
  ...
  body("query.steps")
    ...
    .withMessage("query.steps must be a non-empty array"),
];
...
router.post(
  "/validate",
  [...queryPayloadRules, validateRequest],
  asyncHandler(async (req, res) => {
      ...
      switch (step.type) {
        ...
        case "filter":
          if (!step.field || !step.operator || step.value === undefined) {
            errors.push(
              `Step ${index + 1}: Filter step incomplete (missing field, operator, or value)`,
            );
          }
          break;
        case "aggregate":
          if (!step.field || !step.aggregation) {
            errors.push(
              `Step ${index + 1}: Aggregate step incomplete (missing field or aggregation)`,
            );
          }
          break;
        case "group":
  ...
  for (let i = 0; i < 100; i++) {
    const record: any = {
      ...
      transaction_date: new Date(
        ...
        .toISOString()
        .split("T")[0],
      user_type: ["premium", "basic", "enterprise"][
    ...
    switch (filter.operator) {
      case "gt":
        filteredData = filteredData.filter(
          (record: any) => record[filter.field] > filter.value,
        );
        break;
      case "lt":
        filteredData = filteredData.filter(
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const favorite = {
      ...
      name,
      description:
        description || `Query with ${query.steps?.length || 0} steps`,
      query,
...
router.get(
  "/favorites",
  [query("userId").optional().trim().isLength({ max: 256 }), validateRequest],
  asyncHandler(async (req, res) => {

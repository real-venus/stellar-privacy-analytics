...
async function initializeRateLimiters() {
  ...
  logger.info(
    "Enhanced rate limiters, cache service initialized with Redis and monitoring",
  );

  logger.info(
    "Enhanced rate limiters, cache service and monitoring initialized",
  );
}
  ...
  res.status(200).json({
    ...
    rateLimiting: {
      ...
      metrics: {
        ...
        blockRate: metrics.current
          ? metrics.current.blockedRequests /
...
apiRouter.use(
  "/analytics",
  enhancedRateLimiter
    ? enhancedRateLimiter.enhancedRateLimit({
        enableCollisionDetection: true,
        enableBurstProtection: true,
        enableAdaptiveLimiting: true,
...
initializeServices()
  .then(async () => {
    server.listen(PORT, async () => {
      ...
      logger.info(
        `📊 Metrics available on port ${process.env.METRICS_PORT || 9090}`,
      );
      logger.info(
        `🔒 Privacy-first mode: ${process.env.PRIVACY_MODE || "enabled"}`,
      );
      logger.info(
        `🔐 HSM integration: ${getHSMIntegration().isInitialized() ? "enabled" : "disabled"}`,
      );

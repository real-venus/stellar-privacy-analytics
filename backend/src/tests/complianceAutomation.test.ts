...
describe("Compliance Automation Service", () => {
  describe("Regulations", () => {
    ...
    test("should return undefined for non-existent regulation", () => {
      const regulation =
        complianceAutomationService.getRegulation("non-existent");
  ...
  describe("Compliance Scans", () => {
    ...
    test("should run compliance scan for CCPA", async () => {
      const result =
        await complianceAutomationService.runComplianceScan("ccpa");
    ...
    test("should run compliance scan for HIPAA", async () => {
      const result =
        await complianceAutomationService.runComplianceScan("hipaa");
  ...
  describe("Compliance Dashboard", () => {
    test("should get compliance dashboard", async () => {
      ...
      const dashboard =
        await complianceAutomationService.getComplianceDashboard();
  ...
  describe("Compliance Reports", () => {
    test("should generate compliance report", async () => {
      ...
      const report =
        await complianceAutomationService.generateComplianceReport("gdpr");
...
describe("Compliance Workflow Service", () => {
  describe("Workflow Creation", () => {
    ...
    test("should set due date based on priority", async () => {
      ...
      if (criticalWorkflow.dueDate && lowWorkflow.dueDate) {
        expect(criticalWorkflow.dueDate.getTime()).toBeLessThan(
          lowWorkflow.dueDate.getTime(),
        );
      }
    });
  });
  ...
  describe("Workflow Steps", () => {
    test("should update step status", async () => {
      ...
      const updatedStep = updatedWorkflow.steps.find(
        (s) => s.id === firstStep.id,
      );
  ...
  describe("Workflow Queries", () => {
    test("should get workflow by ID", async () => {
      ...
      const retrieved = await complianceWorkflowService.getWorkflow(
        workflow.workflowId,
      );
    ...
    test("should get workflows by violation", async () => {
      ...
      const workflows =
        await complianceWorkflowService.getWorkflowsByViolation(
          "violation_123",
        );
    ...
    test("should get workflows by status", async () => {
      ...
      const workflows =
        await complianceWorkflowService.getWorkflowsByStatus("pending");
      expect(workflows.length).toBeGreaterThan(0);
      workflows.forEach((w) => {
...
describe("Legal Requirements Service", () => {
  describe("Requirements Retrieval", () => {
    ...
    test("should get requirements by regulation", () => {
      const gdprRequirements =
        legalRequirementsService.getRequirementsByRegulation("gdpr");
    ...
    test("should get specific requirement", () => {
      const requirement =
        legalRequirementsService.getRequirement("gdpr_art_5_1_c");
    ...
    test("should get requirements by category", () => {
      const requirements =
        legalRequirementsService.getRequirementsByCategory("Consumer Rights");
      expect(Array.isArray(requirements)).toBe(true);
    ...
    test("should get requirements by jurisdiction", () => {
      const euRequirements =
        legalRequirementsService.getRequirementsByJurisdiction("EU");
  ...
  describe("Requirements Search", () => {
    ...
    test("should search requirements case-insensitively", () => {
      const lowerResults =
        legalRequirementsService.searchRequirements("consent");
      const upperResults =
        legalRequirementsService.searchRequirements("CONSENT");
  ...
  describe("Requirement Mappings", () => {
    test("should get compliance rule for requirement", () => {
      const ruleId =
        legalRequirementsService.getComplianceRuleForRequirement(
          "gdpr_art_5_1_c",
        );
    ...
    test("should get requirements for compliance rule", () => {
      const requirements =
        legalRequirementsService.getRequirementsForComplianceRule("gdpr-001");
      expect(Array.isArray(requirements)).toBe(true);
      expect(requirements.length).toBeGreaterThan(0);
    });
  });

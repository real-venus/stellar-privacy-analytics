  ...
  const fetchMetrics = async () => {
    try {
      const [metricsResponse] = await Promise.all([
        fetch(
          `/api/v1/service-discovery/metrics${selectedService !== "all" ? `?serviceName=${selectedService}` : ""}`,
        ),
      ]);
      ...
      const mockTimeSeries: TimeSeriesData[] = Array.from(
        { length: 24 },
        (_, i) => ({
          timestamp: `${i}:00`,
  ...
  const getTotalMetrics = (): ServiceMetrics => {
    ...
    return allMetrics.reduce(
      (acc, curr) => ({
        ...
        successRate:
          ((acc.successRequests + curr.successRequests) /
            (acc.totalRequests + curr.totalRequests)) *
          100,
        averageResponseTime:
          (acc.averageResponseTime + curr.averageResponseTime) / 2,
          ...
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="metric-value">
                {totalMetrics.totalRequests.toLocaleString()}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="metric-card">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="metric-value">
                {totalMetrics.successRate.toFixed(1)}%
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="metric-card">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="metric-value">
                {totalMetrics.averageResponseTime}ms
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="metric-card">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="metric-value">
                {totalMetrics.errorRequests.toLocaleString()}
              </p>
            </div>
          </div>
        </div>
                        ...
                        <div className="flex items-center">
                          <div className="flex-1 bg-gray-200 rounded-full h-2 mr-2">
                            <div
                              ...
                              style={{
                                width: `${serviceMetrics.successRate}%`,
                              }}
                            ></div>
                          </div>
        ...
        <div className="metric-card">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span
                className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                  totalMetrics.successRate >= 95
                    ? "status-healthy"
                    : totalMetrics.successRate >= 90
                      ? "status-unknown"
                      : "status-unhealthy"
                }`}
              >
            ...
            <div className="flex items-center justify-between">
              ...
              <span
                className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                  totalMetrics.averageResponseTime <= 100
                    ? "status-healthy"
                    : totalMetrics.averageResponseTime <= 500
                      ? "status-unknown"
                      : "status-unhealthy"
                }`}
              >
            ...
            <div className="flex items-center justify-between">
              ...
              <span
                className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                  totalMetrics.errorRequests / totalMetrics.totalRequests <=
                  0.01
                    ? "status-healthy"
                    : totalMetrics.errorRequests / totalMetrics.totalRequests <=
                        0.05

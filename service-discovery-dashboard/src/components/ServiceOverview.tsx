  ...
  const fetchOverviewData = async () => {
    try {
      ...
      setMetrics({
        ...
        uptime:
          healthData.total > 0
            ? (healthData.healthy / healthData.total) * 100
            : 0,
      });
    ...
    <div className="space-y-6">
            ...
            {alerts.slice(0, 5).map((alert) => (
              ...
              >
                ...
                <div className="text-right">
                  <span
                    className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getSeverityColor(alert.severity)}`}
                  >
                    {alert.severity}
                  </span>
      ...
      <div className="metric-card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <button className="btn-primary">Register New Service</button>
          <button className="btn-secondary">View All Services</button>
          <button className="btn-secondary">System Health Check</button>
        </div>
      </div>
    </div>
  );

...
const FailoverManagement: React.FC = () => {
  ...
  const handleCreatePolicy = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        "/api/v1/service-discovery/failover/policies",
        {
          method: "POST",
          headers: {
  ...
  const handleManualFailover = async (serviceName: string) => {
    if (
      !confirm(
        `Are you sure you want to initiate manual failover for ${serviceName}?`,
      )
    ) {
      return;
    }
    ...
    try {
      const response = await fetch(
        `/api/v1/service-discovery/failover/manual/${serviceName}`,
        {
          method: "POST",
        },
      );
  ...
  const handleManualFailback = async (serviceName: string) => {
    if (
      !confirm(
        `Are you sure you want to initiate manual failback for ${serviceName}?`,
      )
    ) {
      return;
    }
    ...
    try {
      const response = await fetch(
        `/api/v1/service-discovery/failback/manual/${serviceName}`,
        {
          method: "POST",
        },
      );
          ...
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              ...
              <form onSubmit={handleCreatePolicy} className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        failoverStrategy: e.target.value as any,
                      })
                    }
                    className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2"
                  >
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700">
                    Backup Instances (comma-separated)
                  </label>
                  <input
                    type="text"
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        backupInstances: e.target.value
                          .split(",")
                          .map((id) => id.trim())
                          .filter(Boolean),
                      })
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div
                      className={`p-2 rounded-full ${getStateColor(serviceStatus.state)}`}
                    >
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <span className="ml-2 font-medium">
                      {policy.recoveryTimeout}ms
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-500">Backup Instances:</span>
                    <span className="ml-2 font-medium">
                      {policy.backupInstances.length}
                    </span>
                  </div>
                </div>
              </div>

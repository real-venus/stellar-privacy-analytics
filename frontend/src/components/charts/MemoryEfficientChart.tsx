...
const MemoryEfficientChart: React.FC<MemoryEfficientChartProps> = ({
  ...
  const [memoryPressure, setMemoryPressure] = useState<'low' | 'medium' | 'high' | 'critical'>(
    'low'
  );
  ...
  const getMemoryPressureLevel = useCallback(
    (usagePercentage: number): 'low' | 'medium' | 'high' | 'critical' => {
  ...
  const processData = useCallback(
    async (rawData: DataPoint[]) => {
      ...
      processed = memoryAwareSampling(rawData, adjustedMaxPoints, memoryInfo.usagePercentage, {
    ...
    return (
      <div className="bg-white rounded-lg border border-gray-200 p-6" style={{ height }}>
        <div className="flex flex-col items-center justify-center h-full">
          ...
          <p className="text-xs text-gray-500 mt-2">Latest value</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm">
            <div>
              ...
              <span className="font-medium">
                {performanceMetrics.pointsRendered.toLocaleString()}
              </span>
            </div>
            <div>
      ...
      {memoryPressure !== 'low' && (
        ...
        >
          <div className="flex items-center justify-between">
            <span>Memory optimization active - {data.length.toLocaleString()} points rendered</span>
            <span>
              {memoryPressure === 'critical' ? '90%' : memoryPressure === 'high' ? '75%' : '50%'}{' '}
              point reduction
            </span>
          </div>
        </div>

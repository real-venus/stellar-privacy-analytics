...
const MOCK_CONSUMERS: Consumer[] = [
  {
    ...
    permissionLevel: 'Full Analytics',
    activeSince: '2023-10-12',
  },
  {
    id: 'usr_2',
    name: 'Global Health NGO',
    permissionLevel: 'Aggregated Only',
    activeSince: '2023-11-05',
  },
  {
    id: 'usr_3',
    name: 'Stellar Data Indexer',
...
export const DataOwnerControlCenter: React.FC = () => {
  ...
  const handleRevoke = async (consumerId: string | 'ALL') => {
    ...
    try {
      ...
      showToast(
        consumerId === 'ALL' ? 'Emergency Revocation successful.' : 'Access revoked successfully.',
        'success'
      );
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100">
      ...
      {toast.show && (
        <div
          className={`fixed top-4 right-4 p-4 rounded-lg shadow-lg flex items-center space-x-2 text-white ${toast.type === 'success' ? 'bg-green-600' : 'bg-red-600'}`}
        >
            ...
            {consumers.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-6 py-8 text-center text-gray-500">
                  No active consumers have access to your datasets.
                </td>
              </tr>
            ) : (
        ...
        {modalState.step === 1 ? (
          ...
          <div className="text-center py-6">
            ...
            <p className="text-gray-600 text-sm">
              Please sign the transaction using the Freighter wallet prompt to process the
              revocation on the Stellar ledger.
            </p>

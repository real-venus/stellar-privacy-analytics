...
export const DataRetentionSchema = z.object({
  ...
  lastReviewDate: z.string().datetime('Invalid date format'),
});
...
export const PrivacyLevelSchema = z.object({
  id: z.string().min(1, 'Privacy level ID is required'),
  name: z
    .string()
 ...
 */
export function validateSettings<T>(schema: z.ZodSchema<T>, data: unknown): ValidationResult<T> {
  try {
 ...
 */
export function sanitizeSettings<T>(schema: z.ZodSchema<T>, data: unknown): T {
  try {

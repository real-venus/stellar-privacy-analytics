        ...
        {privacyBudget && (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <div
                className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(privacyBudget.status)}`}
              >
            ...
            <div className="relative">
              <div
                className={`w-full ${darkMode ? 'bg-gray-700' : 'bg-gray-200'} rounded-full h-4`}
              >
                <motion.div
                  initial={{ width: 0 }}
                  ...
                  className={`h-4 rounded-full ${
                    privacyBudget.percentageUsed >= 90
                      ? 'bg-red-500'
              ...
              <div className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'} mt-2`}>
                {privacyBudget.percentageUsed.toFixed(1)}% used • Last updated:{' '}
                {new Date(privacyBudget.lastUpdated).toLocaleString()}
              </div>
            </div>
          </motion.div>
        )}
            ...
            {zkProofs.map((proof) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div
                    className={`px-2 py-1 rounded text-xs font-medium ${getStatusColor(proof.status)}`}
                  >
                  ...
                  >
                    <motion.div
                      ...
                      className={`h-2 rounded-full ${
                        proof.status === 'verified' || proof.status === 'completed'
                          ? 'bg-green-500'
                          : proof.status === 'failed'
                            ? 'bg-red-500'
                            : 'bg-blue-500'
                ...
                <div
                  className={`flex items-center justify-between text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}
                >

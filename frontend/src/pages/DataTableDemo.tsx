...
const columns: Column<Row>[] = [
  ...
  { key: 'timestamp', header: 'Timestamp', width: 1.5 },
  {
    ...
    render: (v) => (
      <span
        className={`px-2 py-0.5 rounded-full text-xs font-medium ${v === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
      >
        {String(v)}
      </span>
    ),
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <p className="text-gray-600 mt-1">
          Virtual scrolling — renders only visible rows for smooth performance with large datasets.
        </p>
      </div>
      <VirtualDataTable
        data={data}

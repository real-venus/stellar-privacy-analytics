...
export const PrivacySettings: React.FC = () => {
  ...
  const touch = (key: keyof FormValues) => setTouched((prev) => ({ ...prev, [key]: true }));
  ...
  const Field: React.FC<{ id: keyof FormErrors; label: string; children: React.ReactNode }> = ({
    id,
    label,
    children,
  }) => (
  ...
  return (
    ...
    >
          ...
          {privacyLevels.map((level) => (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <span
                  className={`px-2 py-0.5 text-xs font-medium rounded ${riskColor[level.risk]}`}
                >
                  Risk: {level.risk}
                </span>
              </div>
      ...
      <div className="bg-white rounded-lg shadow p-6 space-y-5">
        ...
        <Toggle
          checked={values.allowDataExport}
          onChange={() => set('allowDataExport', !values.allowDataExport)}
          label="Allow Data Export"
          desc="Permit users to export analysis results"
        />
        <Toggle
          checked={values.allowSharing}
          onChange={() => set('allowSharing', !values.allowSharing)}
          label="Allow Data Sharing"
          desc="Share anonymized insights with partners"
        />
      </div>

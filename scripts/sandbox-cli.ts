...
// Configuration
const API_BASE_URL =
  process.env.STELLAR_API_URL || "http://localhost:3001/api/v1";
...
program
  ...
  .action(async () => {
    try {
      ...
      console.log(
        `Sandbox Mode: ${response.data.sandboxEnabled ? chalk.green("Enabled") : chalk.red("Disabled")}`,
      );
      console.log(
        `Database Schema: ${chalk.yellow(response.data.database.schemaPrefix || "default")}`,
      );
      console.log(
        `Isolation: ${response.data.database.isolationEnabled ? chalk.green("Enabled") : chalk.red("Disabled")}`,
      );
      ...
      console.log("=".repeat(50));
      console.log(
        `RPC URL: ${chalk.cyan(response.data.stellarNetwork.rpcUrl)}`,
      );
      console.log(
        `Horizon URL: ${chalk.cyan(response.data.stellarNetwork.horizonUrl)}`,
      );
      console.log(
        `Network Type: ${chalk.yellow(response.data.stellarNetwork.networkType)}`,
      );
    } catch (error) {
...
program
  ...
  .action(async () => {
    try {
      ...
      const response = await makeRequest("POST", "/sandbox/toggle", {
        enabled: true,
      });
...
program
  ...
  .action(async (environment) => {
    try {
      ...
      const response = await makeRequest("POST", "/sandbox/environment", {
        environment,
      });
...
program
  ...
  .option(
    "-t, --failure-type <type>",
    "Failure type (insufficient_funds, network_error, timeout, invalid_signature)",
    "insufficient_funds",
  )
  .action(async (options) => {
    try {
      const answers = options.subscription
        ? {}
        : await inquirer.prompt([
            {
              ...
              validate: (input) =>
                input.length > 0 || "Subscription ID is required",
            },
          ]);
      ...
      console.log(
        `Status: ${response.data.status === "success" ? chalk.green("Success") : chalk.red("Failed")}`,
      );
  ...
  .action(async (options) => {
    try {
      ...
      response.data.payments.forEach((payment: any) => {
        ...
        console.log(
          `   Status: ${payment.status}${payment.failureType ? ` (${payment.failureType})` : ""}`,
        );
...
program
  ...
  .action(async (type, options) => {
    try {
      ...
      console.log(
        `Event ID: ${chalk.cyan(response.data.eventId || response.data.gracePeriodId || response.data.dunningId)}`,
      );
    } catch (error) {
      log.error(error instanceof Error ? error.message : "Unknown error");
      process.exit(1);
...
program
  ...
  .action(async () => {
    try {
      ...
      // Switch to testnet
      await makeRequest("POST", "/sandbox/environment", {
        environment: "testnet",
      });
